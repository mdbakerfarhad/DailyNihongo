...
import katakanaDataRaw from "../data/katakana.json";
...
const katakanaData: KanaItem[] = katakanaDataRaw as KanaItem[];
...
const STORAGE_KEY = "katakanaScore";

const Katakana: React.FC = () => {
  const [index, setIndex] = useState<number>(() => Math.floor(Math.random() * katakanaData.length));
  ...
  // New state to control mobile feedback in buttons area
  const [showFeedbackInButtonsArea, setShowFeedbackInButtonsArea] = useState(false);

  const item = useMemo(() => katakanaData[index], [index]);
  ...
  const checkAnswer = (e?: React.FormEvent) => {
    ...
    if (isMobile) {
      ...
      setTimeout(() => {
        ...
        setInput("");
        setFeedback(null);
        setIndex((i) => getRandomIndex(katakanaData.length, i));
      }, 1500);
    } else {
      setTimeout(() => {
        setInput("");
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-20 min-h-screen flex flex-col">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-6">
          <div className="text-center flex-1 select-none">
            <div
              ...
              aria-label="Katakana character to practice"
            >
              {item.char}
            </div>
      ...
      <section className="mt-16">
        ...
        <div className="grid grid-cols-5 gap-6">
          {katakanaData.map(({ char, romaji }, idx) => (
            <div
              key={idx}
